...
export function GroupManager({ portfolioId, groups, onUpdate }: GroupManagerProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[400px] border-zinc-200 rounded-none p-0 overflow-hidden font-sans shadow-none">
                <DialogHeader className="px-6 pt-10 pb-6 bg-white border-b border-zinc-100 relative">
                ...
                <div className="p-6 space-y-10 pb-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <form onSubmit={handleCreateGroup} className="flex gap-2">
                            <Input
                                ...
                                disabled={loading}
                                className="h-12 px-4 border-zinc-200 bg-white shadow-none focus-visible:ring-0 focus-visible:border-zinc-400 font-black text-[14px] rounded-none transition-all placeholder:text-zinc-200"
                            />
                            <Button
                                type="submit"
                                disabled={loading || !newGroupName.trim()}
                                className="h-12 px-6 bg-zinc-900 text-white border-none hover:bg-black transition-all rounded-none font-black text-[13px] uppercase tracking-widest shadow-none flex items-center justify-center min-w-[80px]"
                            >
                                {loading ? "..." : "추가"}
                            </Button>
                        </form>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                            {groups.length > 0 ? (
                                groups.map((group) => (
                                    <div key={group.id} className="flex items-center justify-between px-4 py-4 border border-zinc-200 bg-white group transition-all hover:border-zinc-400 rounded-none">
                                        <div className="flex items-center gap-4">
                                            <div className="w-1.5 h-1.5 bg-zinc-200 rounded-full group-hover:bg-zinc-900 transition-colors" />
                                            <span className="text-[13px] font-black text-zinc-800 uppercase tracking-tight">{group.name}</span>
                                        </div>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className="h-8 w-8 p-0 text-zinc-200 hover:text-rose-500 hover:bg-white bg-white rounded-none opacity-0 group-hover:opacity-100 transition-all"
