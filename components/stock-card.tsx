...
export function StockDetailView({ stock, groups = [], onUpdate, onDelete }: StockDetailViewProps) {
  ...
  return (
    <div className="space-y-16 h-full flex flex-col font-sans">
      {/* Page Header - Ultra Minimal */}
      <div className="flex flex-col gap-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-4">
              <h2 className="text-4xl font-black tracking-tighter text-zinc-900 uppercase">{stock.name}</h2>
              {stock.symbol && (
                <span className="px-3 py-1 bg-zinc-100 text-zinc-500 rounded-none text-[12px] font-black uppercase tracking-[0.2em]">
                  {stock.symbol}
                </span>
              )}
            </div>
            <p className="text-[14px] font-black text-zinc-400 uppercase tracking-[0.2em]">자산 세부 정보 및 거래 내역</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 bg-zinc-200 border border-zinc-200 gap-px">
          {[
            ...
            <div key={i} className="bg-white p-6 lg:p-8 flex flex-col gap-2">
              <span className="text-[13px] font-black text-zinc-400 uppercase tracking-[0.2em]">{item.label}</span>
              <span className={cn(
      ...
      {/* Stats - Grid Style */}
      <div className="grid grid-cols-1 md:grid-cols-4 bg-zinc-200 border border-zinc-200 gap-px">
        {[
          ...
          <div key={idx} className="bg-white p-6 lg:p-8 flex flex-col gap-4">
            <span className="text-[13px] font-black text-zinc-400 uppercase tracking-[0.2em]">{metric.label}</span>
            <div className={cn(
              "text-2xl font-black tracking-tighter",
